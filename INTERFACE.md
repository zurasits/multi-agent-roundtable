...
Returns the current discussion transcript.

### `submit_message(agent_id: str, content: str) -> None`
Allows the frontend to submit a message on behalf of a live user or an external source.

### `trigger_roundtable_step() -> None`
Instructs the backend to execute one turn of the multi-agent discussion, invoking the respective agents.

## 4. Demo Data Contract
The `demo_data/transcript_001.json` file must contain a JSON array of `Message` objects.